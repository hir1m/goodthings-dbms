...
const UpdateAvaliableItem: NextPage<Props> = ({ userdata }) => {
  ...
  const handleClick = async (_: MouseEvent<HTMLButtonElement>) => {
    const { id, price, quantity, category } = formData;
    ...
    if (id.length < 1) {
      ...
      return;
    } else if (price.length < 1 && quantity.length < 1 && category.length < 1) {
      setErrorMsg("Error: at least one of the fields has to be not empty");
      return;
    } else if (price.length >= 1 && !isFloat(price)) {
      setErrorMsg("Error: price has to be a decimal number");
      return;
    } else if (quantity.length >= 1 && !isInteger(quantity)) {
      setErrorMsg("Error: quantity has to be a integer");
      return;
    }

    try {
      const res = await jwtFetch(
        "/api/update/avaliable_item",
        jwt_token,
        {
  ...
  return (
    <div>
      <Modal show={ok} centered>
        ...
        <Modal.Body>Item with id:{formData.id} is updated</Modal.Body>
        <Modal.Footer>
          <Button
            variant="primary"
            onClick={(_) => {
      ...
      <Container>
        ...
        >
          <h3 className="text-center text-md-left">Edit an item</h3>
        </Row>
      </Container>
      ...
      <Container>
        <Form>
          ...
          <Form.Group className={styles.form_group} as={Row} controlId="price">
            <Form.Label column xs="3">
              &nbsp;Price
            </Form.Label>
            <Col xs="9">
...
UpdateAvaliableItem.getInitialProps = async (ctx) => {
...
export default UpdateAvaliableItem;
